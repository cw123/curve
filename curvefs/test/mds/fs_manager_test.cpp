...
TEST_F(FSManagerTest, test1) {
    ...
    EXPECT_CALL(mockSpaceService_, InitSpace(_, _, _, _))
        .WillOnce(
            DoAll(SetArgPointee<2>(initSpaceResponse),
                  Invoke(RpcService<InitSpaceRequest,
                                    InitSpaceResponse>)));
    ...
    ASSERT_TRUE(CompareMountPoint(fsInfo3.mountpoints(0), mountPoint));
    ...
    ASSERT_TRUE(CompareMountPoint(fsInfo4.mountpoints(0), mountPoint));
